...
class KMSKeyManager:
    ...
    def get_aes_key(self, filter_name, force_refresh=False):
        try:
            ...
            logger.info("AES key generated and cached for filter: %s", filter_name)
            return aes_key
        except Exception as e:
            logger.error("Failed to retrieve AES key for filter %s: %s", filter_name, e)
            raise
    ...
    def get_key_id(self, filter_name):
        ...
        if os.path.exists(key_id_file):
            ...
            # Logic to retrieve the key ID from KMS if the file doesn't exist
