...
- `encryption_scheme` in `config.ini` (`v1`|`v2`) sets the scheme new keys get when `setup-aes-key` is run without `--scheme`. Precedence: `--scheme` flag > config `encryption_scheme` > built-in `v2`. Invalid values raise at load.
