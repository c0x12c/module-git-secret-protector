...
- `encryption_scheme` in `config.ini` (`v1`|`v2`) sets the scheme new keys get when `setup-aes-key` is run without `--scheme`. Precedence: `--scheme` flag > config `encryption_scheme` > built-in `v1`. Built-in default is v1 (legacy unauthenticated CBC) for widest client compatibility across mixed-version fleets; set `encryption_scheme = v2` for authenticated encryption. Invalid values raise at load.
...
- `AesEncryptionHandler` - authenticated deterministic encryption. HKDF-SHA256 derives domain-separated enc/mac/iv subkeys from the stored key; IV is content-derived (`HMAC(iv_key, plaintext)[:16]`) so identical plaintext yields identical ciphertext (git-stable); AES-256-CTR + encrypt-then-HMAC-SHA256. Wire format `magic_header(default ENCRYPTED) + 0x02 + base64(iv||ct||tag)`. The **encrypt** scheme is per-filter, selected from the key blob's `version` field (1=legacy CBC, 2=this scheme) via `AesKeyManager.get_scheme`; the scheme new keys get is resolved by precedence `--scheme` flag > config `encryption_scheme` > built-in v1 (v1 default = widest cross-version compat; opt up to v2 in config). **Decrypt is version-byte-authoritative and independent of the blob scheme** (dispatches on the wire byte, and fails closed): `0x02`→v2; a base64-alphabet first byte (v1 has no version byte)→v1 legacy AES-CBC; any other byte→hard error (unrecognized format, not silent CBC fallthrough). So a v2 key still decrypts on-disk v1 files mid-migration. Ceiling: the v1-vs-newer disambiguation holds while version markers stay outside the base64 alphabet (control bytes today). Truncated v2 payloads (`< 48` bytes) are rejected before slicing. `get_scheme` maps version 1/2 and fails closed on `>= 3` (a **missing** field defaults to **v1**: every blob writer - setup/set_scheme/rotate - emits an explicit `version`, so a version-less blob is definitionally a pre-versioning v1-era blob; defaulting it to v2 made the clean filter emit v2 for v1 committed ciphertext, leaving decrypted files permanently dirty - fixed in 1.9.0). `get_scheme_info` additionally returns whether the version field was present, so `doctor` can flag a version-less (silently-v1) blob distinctly. `upgrade-scheme` re-encrypts v1→v2.
